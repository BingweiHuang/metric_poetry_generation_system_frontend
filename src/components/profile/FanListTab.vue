<template>
  <!-- 粉丝列表 -->
    <ul v-infinite-scroll="fan_load" infinite-scroll-distance="1" infinite-scroll-immediate="false"
            ...
            <li v-for="(item, index) in fans.list" :key="'fan_' + item.id + index" class="infinite-list-item">
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; width: 100%">
                    <div style="display: flex; align-items: center">
                        <!-- 头像 -->
                        <el-avatar :size="50" :src="item.fan.avatar_url" style="margin-right: 10px; cursor: pointer"
                                   @click="open_profile(item.fan.id)"/>
                        <!-- 昵称-->
                        <div>
                            <span style="cursor: pointer; font-size: 18px"
                                  @click="open_profile(item.fan.id)">{{ item.fan.nickname }}</span>
                        </div>
                    </div>
        ...
        <li v-else style="text-align: center; color: #6b778c; margin-top: 10px">还没有粉丝~</li>
