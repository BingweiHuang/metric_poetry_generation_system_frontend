<template>
  <!-- 关注列表 -->
    <ul v-infinite-scroll="follow_load" infinite-scroll-distance="1" infinite-scroll-immediate="false"
        class="infinite-list" style="overflow: auto; max-height: 520px;">
            ...
            <li v-for="(item, index) in follows.list" :key="'follow_' + item.id + index" class="infinite-list-item">
                ...
                <div style="display: flex; align-items: center; justify-content: space-between; width: 100%">
                    <div style="display: flex; align-items: center">
                        <!-- 头像 -->
                        <el-avatar :size="50" :src="item.follow.avatar_url" style="margin-right: 10px; cursor: pointer"
                                   @click="open_profile(item.follow.id)"/>
                        <!-- 昵称-->
                        <div>
                            <span style="cursor: pointer; font-size: 18px"
                                  @click="open_profile(item.follow.id)">{{ item.follow.nickname }}</span>
                        </div>
                    </div>
                    <template v-if="is_me">
                        <el-button v-if="item.id === 0" type="success" @click="follow_list_add(item.follow.id, index)">关
                            注
                        </el-button>
