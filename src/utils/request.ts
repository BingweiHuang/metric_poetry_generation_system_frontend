...
import {closeLoading, Loading} from "@/utils/loading_instance";
...
// export const system_base_url = 'http://82.156.236.172:8080/'; // 直接访问后端服务器
export const system_base_url = '/sys/'; //nginx 反向代理
// export const AI_base_url = 'http://60.205.235.222:8080/'; // 直接访问后端服务器
export const AI_base_url = '/ai/'; //nginx 反向代理


// export const trans_next_url = 'http://bwhlifetimelove.top/';


const indx2text = ['', '作诗中', '邮件发送中', '努力加载中']

// 创建 axios 实例
const auth_instance = axios.create({
    // baseURL: 'http://82.156.236.172:8080', // 直接访问后端服务器
    // baseURL: 'http://bwhlifetimelove.top/', // 配合 system_base_url='/sys/'的情况 访问代理服务器
    timeout: 20 * 1000,
    headers: {
...
const httpErrorStatusHandle = (error) => {
    ...
    switch (error.response.status) {
        ...
        case 429:
            ...
            break;
        case 500:
            message = error.response.data.result ? error.response.data.result : '服务器内部错误！';
            break;
        case 501:
            message = '服务未实现！';
            break;
